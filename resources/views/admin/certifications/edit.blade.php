...
<div class="max-w-4xl mx-auto">
    <!-- Header -->
    <div class="mb-6">
        <h1 class="text-3xl font-bold text-rose-pine-text">Edit Certification</h1>
        <p class="text-rose-pine-subtle mt-2">Update the details for {{ $certification->title }}.</p>
    </div>
    ...
    <form action="{{ route('admin.certifications.update', $certification->id) }}" method="POST" class="space-y-6">
        ...
        <div>
            <x-admin-image-upload
                ...
                label="Badge Image"
                :current="$certification->badge_url"
            />
            @error('badge_image')
            <p class="mt-1 text-sm text-rose-pine-love">{{ $message }}</p>
            @enderror
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div>
                ...
                <input type="date"
                       name="expires_at"
                       id="expires_at"
                       value="{{ old('expires_at', $certification->expires_at?->format('Y-m-d')) }}"
            ...
            <div class="flex items-center">
                <label class="flex items-center space-x-2 cursor-pointer">
                    <input type="checkbox"
                           ...
                           id="is_featured"
                           value="1"
                           {{ old('is_featured', $certification->is_featured) ? 'checked' : '' }}
