        ...
        <div class="flex-1 min-w-0">
            <div class="flex items-center gap-2 flex-wrap">
                ...
                @if($comment->is_highlighted)
                    <span class="inline-flex items-center px-2 py-0.5 rounded text-xs bg-rose-pine-gold text-gray-900" title="Highlighted comment">
    ...
    <div class="flex flex-wrap items-center gap-2">
        @if($comment->status !== 'approved')
            <form action="{{ route('admin.comments.approve', $comment) }}" method="POST" class="inline">
                @csrf
                <button type="submit" 
                        class="inline-flex items-center gap-2 px-4 py-2 bg-rose-pine-foam text-gray-900 rounded-lg hover:bg-opacity-80 transition-colors text-sm font-medium">
                    <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
        ...
        @if($comment->status !== 'spam')
            <form action="{{ route('admin.comments.spam', $comment) }}" method="POST" class="inline">
                @csrf
                <button type="submit" 
                        class="inline-flex items-center gap-2 px-4 py-2 bg-rose-pine-gold text-gray-900 rounded-lg hover:bg-opacity-80 transition-colors text-sm font-medium">
                    <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
        ...
        <form action="{{ route('admin.comments.destroy', $comment) }}" method="POST" class="inline"
            ...
            @method('DELETE')
            <button type="submit" 
                    class="inline-flex items-center gap-2 px-4 py-2 bg-rose-pine-love text-white rounded-lg hover:bg-opacity-80 transition-colors text-sm font-medium">
                <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
